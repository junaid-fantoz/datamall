import React, { useEffect, useRef, useState } from "react";
import './Select.css';

const ArrowIcon = () => {
  return (
    <svg height="20" width="20" viewBox="0 0 20 20">
      <path d="M4.516 7.548c0.436-0.446 1.043-0.481 1.576 0l3.908 3.747 3.908-3.747c0.533-0.481 1.141-0.446 1.574 0 0.436 0.445 0.408 1.197 0 1.615-0.406 0.418-4.695 4.502-4.695 4.502-0.217 0.223-0.502 0.335-0.787 0.335s-0.57-0.112-0.789-0.335c0 0-4.287-4.084-4.695-4.502s-0.436-1.17 0-1.615z"></path>
    </svg>
  );
};

const CloseIcon = () => {
  return (
    <svg height="20" width="20" viewBox="0 0 20 20">
      <path d="M14.348 14.849c-0.469 0.469-1.229 0.469-1.697 0l-2.651-3.030-2.651 3.029c-0.469 0.469-1.229 0.469-1.697 0-0.469-0.469-0.469-1.229 0-1.697l2.758-3.15-2.759-3.152c-0.469-0.469-0.469-1.228 0-1.697s1.228-0.469 1.697 0l2.652 3.031 2.651-3.031c0.469-0.469 1.228-0.469 1.697 0s0.469 1.229 0 1.697l-2.758 3.152 2.758 3.15c0.469 0.469 0.469 1.229 0 1.698z"></path>
    </svg>
  );
};

const Select = ({ isMulti, placeHolder, options, isSearchable, onChange }) => {
  const [showOptions, setShowOptions] = useState(false);
  const [selectedOption, setSelectedOption] = useState(isMulti ? [] : null);
  const [searchValue, setSearchValue] = useState("");
  const searchRef = useRef();

  console.log('options', options);

  const handleSelectInputClick = (e) => {
    e.stopPropagation();
    setShowOptions(!showOptions);
  };

  useEffect(() => {
    setSearchValue("");
    if (showOptions && searchRef.current) {
      searchRef.current.focus();
    }
  }, [showOptions]);

  const getOptions = () => {
    if (!selectedOption || selectedOption.length === 0) {
      return placeHolder;
    }

    if (isMulti) {
      return (
        <div className="select-tags">
          {selectedOption.map((option) => {
            return <div key={option.value} className="select-tag-item">
              {option.label}
              <span
                onClick={(e) => onTagRemove(e, option)}
                className="select-tag-close"
              >
                <CloseIcon />
              </span>
            </div>;
          })}
        </div>
      );
    }
    return selectedOption.label;
  };

  const removeOption = (option) => {
    return selectedOption.filter((o) => o.value !== option.value);
  };

  const onTagRemove = (e, option) => {
    e.stopPropagation();
    const newValue = removeOption(option);
    setSelectedOption(newValue);
    onChange(newValue);
  };

  const onItemClick = (option) => {
    let newValue;
    if (isMulti) {
      if (selectedOption.findIndex((o) => o.value === option.value) >= 0) {
        newValue = removeOption(option);
      } else {
        newValue = [...selectedOption, option];
      }
    } else {
      newValue = option;
    }
    console.log('selected option', option, newValue);
    setSelectedOption(newValue);
    onChange(newValue);
  };

  const isOptionSelected = (option) => {
    console.log('is option selected', selectedOption, option, isMulti);
    if (isMulti) {
      return selectedOption.filter((o) => o.value === option.value).length > 0;
    }

    if (!selectedOption) {
      return false;
    }

    return selectedOption.value === option.value;
  };

  const onSelectSearch = (e) => {
    setSearchValue(e.target.value);
  };

  const getSearchOptions = () => {
    if (!searchValue) {
      return options;
    }

    return options.filter(
      (option) =>
        option.label.toLowerCase().indexOf(searchValue.toLowerCase()) >= 0
    );
  };

  return (
    <div className="select-container">
      <div onClick={handleSelectInputClick} className="select-input">
        <div className="selected-value">{getOptions()}</div>
        <div className="select-arrow-icon">
          <div className="arrow-icon">
            <ArrowIcon />
          </div>
        </div>
      </div>
      {showOptions && (
        <div className="select-options">
          {isSearchable && (
            <div className="search-box">
              <input
                onChange={onSelectSearch}
                value={searchValue}
                ref={searchRef}
              />
            </div>
          )}
          {getSearchOptions().map((option) => {
            return <div
              onClick={() => onItemClick(option)}
              key={option.value}
              className={`select-item ${
                isOptionSelected(option) && "selected"
              }`}
            >
              {option.label}
            </div>;
          })}
        </div>
      )}
    </div>
  );
};

export default Select;
